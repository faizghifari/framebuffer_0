...
const char* title_text = "Dery Rahman";
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        for (i = 0; i < strlen(title_text); i++) {
            ...
            put_image_color(filename, backbuff, 10+(char_width + 2)*i, cur_y, 0xff, 0x00, 0xff, &var_screen_info, &fix_screen_info);
            free(filename);
        }
        
        memcpy(fbp, backbuff, var_screen_info.yres_virtual * fix_screen_info.line_length);

        cur_y = (cur_y + 4) % (var_screen_info.yres - char_width);
        usleep(10);
    }
